...
options:
  ...
  state:
    ...
    required: false
    default: present
    choices: [ "present", "absent", "build", "pull" ]
    aliases: []
  timeout:
    description:
...
Fetch "my/app" image from docker registry:

- hosts: web
  sudo: yes
  tasks:
  - name: check or build image
    docker_image: name="my/app" state=pull
...
class DockerImageManager:
    ...
    def pull(self):
        try:
            changes = list(self.client.pull(self.name, tag=self.tag, stream=True))
            try:
                last = changes[-1]
            except IndexError:
                last = '{}'
            ...
            self.module.fail_json(msg="Failed to pull the specified image: %s" % self.name, error=repr(e))
...
def main():
    module = AnsibleModule(
        argument_spec = dict(
            ...
            state              = dict(default='present', choices=['absent', 'present', 'build', 'pull']),
            pull              = dict(required=False, default=False),
